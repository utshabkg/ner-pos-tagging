...
def combine_word_ner_pos(data):
    tuples_fun = lambda s: [(word, pos, ner) for word, pos, ner in
                            zip(s['Token'].values.tolist(), s['POS'].values.tolist(), s['NER'].values.tolist())]
...
def save_data(train_x, test_x, train_y_pos, test_y_pos, train_y_ner, test_y_ner, words_vocab, pos_vocab, ners_vocab,
              words, poss, ners, output_dir):
